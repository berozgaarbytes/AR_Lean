// Wrap everything in a "Wait for UI" block to fix your Error
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function bootSequence() {
        ...
        try {
            ...
            uiOverlay.style.display = 'none'; // Now this will definitely work
            renderLoop();
    ...
    function audit(d) {
        ...
        d.mass = variance > 12 ? "LOW_DENSITY (EMPTY)" : "HIGH_DENSITY (FULL)";
        d.vol = Math.round((d.w * d.h * ((d.w+d.h)/2)) / 12000);
    }
        ...
        for (let i = 0; i < data.length; i += 85) {
            ...
            if (score > 0.4) {
                ...
                if (score * max > 0.4) {
                    ...
                    boxes.push({ x:(cx-w/2)*(vW/416), y:(cy-h/2)*(vH/416), w:w*(vW/416), h:h*(vH/416), score:score*max, label:CLASSES[id] });
                }
            }
        }
        return boxes;
